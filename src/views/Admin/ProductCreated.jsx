...
const ProductCreated = () => {
  ...
  useEffect(() => {
    const sortedProducts = handleSort(adminProducts, sortBy);
    const paginatedProducts = handlePagination(sortedProducts, currentPage, pageSize);
    setFilteredProducts(paginatedProducts);
  }, [adminProducts, sortBy, currentPage, pageSize]);
  ...
  useEffect(() => {
    const filtered = filterProducts(
      adminProducts.flatMap((purchase) => purchase),
      nameFilter,
      priceMin,
      priceMax
    );
      ...
      <table className="table text-gray-400 text-center border-separate space-y-6 text-sm">
        <thead className="bg-gray-800 text-gray-500">
          <tr>
            ...
            <th className="p-3 text-gray-300 w-screen text-center">Fecha de creación</th>
          </tr>
        </thead>
        <tbody>
